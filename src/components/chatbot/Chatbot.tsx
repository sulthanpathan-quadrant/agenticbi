...
const Chatbot = ({ onShowAnalysis, onDatasetUploaded }: ChatbotProps) => {
  ...
  const pollDatasetKpisStatus = async (
    jobId: string,
    userEmail: string,
      ...
      const poll = async () => {
        try {
          const pollResp = await fetch(
            `https://api.veriton.ai/api/service3/dataset-kpis-status/${jobId}?user_email=${encodeURIComponent(
              userEmail,
            )}`,
          ...
          if (!pollResp.ok) {
            throw new Error("Dataset KPI status polling failed");
          }

          const result = await pollResp.json();

          if (result.status === "success" && result.available) {
            resolve(result);
          } else if (result.status === "failed" || result.status === "error") {
            reject(result);
          } else {
            // status is "started" or "running" — keep polling
  ...
  const fetchDatasetKpis = async (filePath: string, userEmail: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          startData.message || "Failed to start dataset KPI generation",
        );
      }

      let kpiData = startData;

      // dataset_kpis now returns { status: "started", job_id, poll_every_seconds }
      // instead of the final KPI payload directly — poll until it's done.
      if (startData.status === "started" && startData.job_id) {
        kpiData = await pollDatasetKpisStatus(
          startData.job_id,
          userEmail,
      ...
      if (!kpiData || !kpiData.available) {
        throw new Error("KPI generation did not return usable data.");
      }
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed right-4 bottom-4 w-[38vw] min-w-[520px] max-w-[680px] z-[150] flex rounded-2xl overflow-hidden border border-border bg-card shadow-xl "
            style={{
