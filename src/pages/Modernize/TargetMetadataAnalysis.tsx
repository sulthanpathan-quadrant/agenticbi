 ...
 * same approach as SourceMetadataResult.
 *
 * `description` on both the table and each column is optional:
 * the backend only fills it in when it has something meaningful
 * to say, so it can be present on some tables/columns and absent
 * on others (or missing entirely from an older response) — the
 * UI below treats it as fully optional at every level.
 */
...
/*
 * One table's entry in the catalog: table name + description (if any),
 * then only the columns that actually carry a description — columns
 * with none are simply omitted rather than shown with an empty line.
 *
 * Each column is laid out as a two-column row: the name sits in a
 * fixed-width gutter on the left and its description runs alongside
 * it on the right, so the descriptions line up down the card.
 */
function TargetCatalogTable({ table }: { table: TargetMetadataTable }) {
  const describedColumns = table.columns.filter((col) => !!col.description);
  ...
  return (
    ...
    >
      ...
      {describedColumns.length > 0 && (
        <div className="mt-3 max-h-56 space-y-2 overflow-y-auto border-t border-border pt-3 pr-1">
          {describedColumns.map((col) => (
            <div key={col.name} className="flex items-baseline gap-3">
              <span
                className="w-40 shrink-0 truncate font-mono text-xs font-semibold text-foreground"
                title={col.name}
              >
                {col.name}
              </span>
...
/*
 * Fully dynamic: descriptions may or may not be present on any given
 * response. Filters out tables with nothing to show, and renders
 * nothing at all (not even the heading) if no table in the whole
 * result carries a table- or column-level description.
 */
function TargetCatalogSection({ tables }: { tables: TargetMetadataTable[] }) {
  const describedTables = tables.filter(
    (table) =>
      !!table.description || table.columns.some((col) => !!col.description)
  );

  if (describedTables.length === 0) return null;

  return (
    <div className="mt-6">
      <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
        Target Catalog ({describedTables.length})
      </h3>
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-[360px_1fr]">
            <div className="flex flex-col gap-4">
              <div>
                <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Target Tables ({targetMetadata.tables.length})
                </h3>
                <div className="max-h-[270px] space-y-3 overflow-y-auto pr-1">
                  {targetMetadata.tables.map((table) => (
                    <TargetTableListItem
                  ...
                  {targetMetadata.tables.length === 0 && (
                    <p className="rounded-xl border border-border px-4 py-6 text-center text-xs text-muted-foreground">
                      No tables found.
                    </p>
                  )}
                </div>
              ...
              <div>
                <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Target Relationships ({targetMetadata.relationships.length})
                </h3>
                <div className="max-h-[270px] overflow-y-auto rounded-xl border border-border">
                  {targetMetadata.relationships.map((relationship) => (
                    <TargetRelationshipListItem
          ...
          {/*
            Renders only when the API response actually includes
            table/column descriptions — invisible otherwise, so
            older or description-less responses look unchanged.
          */}
          <TargetCatalogSection tables={targetMetadata.tables} />
        </>
      )}
