...
// import { sampleFiles } from '@/components/data/sampleFiles';
// import { generateDashboardData } from '@/components/utils/generateDashboardData';
// import { DataFile, DashboardData, KPI } from '@/components/types/dashboard';
...
//   const handleBuildWithRecommendations = async (kpis: KPI[]) => {
//     if (!selectedFile) return;
   
//     const kpiLabels = kpis.map(k => k.label).join(', ');
//     const query = `Dashboard with recommended KPIs: ${kpiLabels}`;
//     await generateDashboard(query, kpis);
//   };
...
//   const handleChatbotGenerate = async (query: string) => {
//     if (!selectedFile) return;
//     await generateDashboard(query);
//   };
...
//   const generateDashboard = async (query: string, selectedKPIs?: KPI[]) => {
//     if (!selectedFile) return;
...
//     const data = generateDashboardData(selectedFile, query);
   
//     // If KPIs were selected, use them instead of generated ones
//     if (selectedKPIs && selectedKPIs.length > 0) {
//       data.kpis = selectedKPIs.slice(0, 4);
//     }
...
//           <ChatbotInterface
//             file={selectedFile}
//             onGenerateDashboard={handleChatbotGenerate}
...
//             <p className="text-sm text-muted-foreground mt-2">Analyzing data patterns and creating visualizations</p>
//           </div>
//         </div>
//       )}
//     </div>
...
const PowerBIDashboard = () => {
  ...
  const handleCustomGenerate = async (query: string) => {
    ...
    try {
      const response = await fetch('http://20.81.213.147:8000/generate_dashboard_from_prompt', {
        method: 'POST',
