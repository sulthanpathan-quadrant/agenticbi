...
//   CheckCircle, Cpu, Calendar, GitBranch, Clock,
//   Database, Search, Eye, ChevronDown, ChevronUp,
//   Table2, PlayCircle, Star, ChevronRight,
// } from "lucide-react";
// import { WorkflowLayout } from "@/components/WorkflowLayout";
// import ReactFlow, {
//   Controls,
...
// // EditableField
// // ─────────────────────────────────────────────────────────────
// function EditableField({ value, onSave, label, saving }: {
//   value: string; onSave: (v: string) => Promise<void>; label: string; saving?: boolean;
// }) {
//   const [editing, setEditing] = useState(false);
//   const [draft, setDraft] = useState(value);
//   const [busy, setBusy] = useState(false);
...
//   return (
//     <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
//       <span style={{ fontSize: 12, color: "hsl(var(--muted-foreground))", fontWeight: 600 }}>{label}:</span>
//       {editing ? (
//         <>
//           <input ref={inputRef} value={draft} onChange={(e) => setDraft(e.target.value)}
//             onKeyDown={(e) => { if (e.key === "Enter") handleSave(); if (e.key === "Escape") handleCancel(); }}
//             disabled={busy}
//             style={{ fontSize: 13, fontWeight: 600, color: "hsl(var(--foreground))", background: "hsl(var(--background))", border: "1.5px solid hsl(var(--primary))", borderRadius: 6, padding: "2px 8px", outline: "none", minWidth: 160 }}
//           />
//           {busy ? <Loader2 style={{ width: 14, height: 14 }} className="animate-spin" /> : (
//             <>
//               <button onClick={handleSave} style={{ background: "hsl(142 72% 42%)", border: "none", borderRadius: 5, padding: "2px 6px", cursor: "pointer", display: "flex", alignItems: "center" }}><Check style={{ width: 12, height: 12, color: "#fff" }} /></button>
//               <button onClick={handleCancel} style={{ background: "hsl(0 72% 51%)", border: "none", borderRadius: 5, padding: "2px 6px", cursor: "pointer", display: "flex", alignItems: "center" }}><XCircle style={{ width: 12, height: 12, color: "#fff" }} /></button>
//             </>
//           )}
//         </>
//       ) : (
//         <>
//           <span style={{ fontSize: 13, fontWeight: 600, color: "hsl(var(--foreground))" }}>{value}</span>
//           <button onClick={() => { setDraft(value); setEditing(true); }} style={{ background: "hsl(var(--muted))", border: "1px solid hsl(var(--border))", borderRadius: 5, padding: "2px 6px", cursor: "pointer", display: "flex", alignItems: "center", gap: 3 }}>
//             <Pencil style={{ width: 11, height: 11, color: "hsl(var(--muted-foreground))" }} />
//             <span style={{ fontSize: 10, color: "hsl(var(--muted-foreground))", fontWeight: 600 }}>Edit</span>
//           </button>
//         </>
...
//         <div style={{ background: "linear-gradient(135deg, hsl(267 84% 60%), hsl(220 90% 60%))", borderRadius: 6, padding: "3px 10px", display: "flex", alignItems: "center", gap: 5 }}>
//           <span style={{ fontSize: 11 }}>💡</span>
//           <span style={{ fontWeight: 700, color: "#fff", fontSize: 11 }}>What this diagram shows</span>
//         </div>
//       </div>
//       <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
//         <span style={{ width: 9, height: 9, borderRadius: "50%", background: "hsl(267 84% 65%)", display: "inline-block", flexShrink: 0 }} />
...
//         <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
//           <span style={{ fontSize: 13 }}>🔗</span>
//           <span style={{ fontWeight: 700, fontSize: 12 }}>How they connect</span>
//         </div>
//         <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
//           {relationships.map((rel, i) => (
//             <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", background: "hsl(var(--background) / 0.5)", border: "1px solid hsl(267 84% 65% / 0.15)", borderRadius: 8, padding: "5px 10px" }}>
...
//   const isFact = data.type === "FACT";
//   return (
//     <div style={{ background: "hsl(var(--card))", border: `2px solid ${isFact ? "hsl(var(--primary))" : "hsl(var(--border))"}`, borderRadius: 10, minWidth: 180, boxShadow: "0 4px 16px rgba(0,0,0,0.15)" }}>
//       <Handle type="target" position={Position.Left} style={{ background: "transparent", border: 0 }} />
//       <Handle type="source" position={Position.Right} style={{ background: "transparent", border: 0 }} />
//       <div style={{ background: isFact ? "hsl(var(--primary))" : "hsl(var(--muted))", borderRadius: "8px 8px 0 0", padding: "6px 10px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
//         <span style={{ fontWeight: 700, fontSize: 12, color: isFact ? "#fff" : "hsl(var(--foreground))" }}>{data.label}</span>
...
// interface PipelineCreatedResult { pipeline_id: string; user_id: string; name: string; jobs: string[]; schedule?: { type: string; hour: number; minute: number; day?: string }; job_names?: Record<string, string>; }
// interface Message { id: string; role: "user" | "assistant"; content: string; result?: MessageResult; dashboardResult?: DashboardResult; automlResult?: AutoMLResult; pipelineCreated?: PipelineCreatedResult; attachment?: string; error?: boolean; timestamp: Date; pipelineJobs?: PipelineJob[]; isDatasetPicker?: boolean; }
...
//         <thead>
//           <tr style={{ background: "hsl(var(--muted))" }}>
//             <th style={{ padding: "8px 12px", textAlign: "left", fontWeight: 700, color: "hsl(var(--foreground))", borderBottom: "1px solid hsl(var(--border))", whiteSpace: "nowrap" }}>Model</th>
//             {metricSpecs.map((m) => (<><th key={`${m.key}-train`} style={{ padding: "8px 10px", textAlign: "center", fontWeight: 600, color: "hsl(var(--muted-foreground))", borderBottom: "1px solid hsl(var(--border))", whiteSpace: "nowrap" }}>{m.label} (train)</th><th key={`${m.key}-test`} style={{ padding: "8px 10px", textAlign: "center", fontWeight: 600, color: "hsl(var(--muted-foreground))", borderBottom: "1px solid hsl(var(--border))", whiteSpace: "nowrap" }}>{m.label} (test)</th></>))}
//           </tr>
//         </thead>
//         <tbody>
//           {modelNames.map((modelName, idx) => {
...
//             const isBest = modelName === bestModel;
//             return (
//               <tr key={modelName} style={{ background: isBest ? "hsl(142 72% 42% / 0.08)" : idx % 2 === 0 ? "transparent" : "hsl(var(--muted) / 0.3)", borderBottom: "1px solid hsl(var(--border) / 0.5)" }}>
//                 <td style={{ padding: "8px 12px", fontWeight: 600, whiteSpace: "nowrap" }}>
//                   <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
//                     <span style={{ color: "hsl(var(--foreground))" }}>{modelName.replace(/_/g, " ")}</span>
//                     {isBest && <span style={{ fontSize: 9, fontWeight: 700, color: "#fff", background: "hsl(142 72% 38%)", borderRadius: 4, padding: "1px 5px", lineHeight: 1.6 }}>BEST</span>}
//                   </div>
//                 </td>
//                 {metricSpecs.map((m) => (<><td key={`${m.key}-train`} style={{ padding: "8px 10px", textAlign: "center", color: "hsl(var(--muted-foreground))", fontVariantNumeric: "tabular-nums" }}>{fmt(modelData?.train?.[m.key])}</td><td key={`${m.key}-test`} style={{ padding: "8px 10px", textAlign: "center", color: "hsl(var(--foreground))", fontWeight: isBest ? 600 : 400, fontVariantNumeric: "tabular-nums" }}>{fmt(modelData?.test?.[m.key])}</td></>))}
//               </tr>
//             );
//           })}
//         </tbody>
...
//         if (!trimmed) return <div key={i} style={{ height: 6 }} />;
//         if (trimmed.startsWith("|")) return null;
//         if (/^#{1,3}\s/.test(trimmed)) return <div key={i} style={{ fontWeight: 700, fontSize: 13, marginTop: 14, marginBottom: 4 }}>{trimmed.replace(/^#{1,3}\s/, "")}</div>;
//         if (/^[-\*•]\s/.test(trimmed)) return <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 8, marginBottom: 4 }}><span style={{ color: "hsl(var(--primary))", fontWeight: 700, fontSize: 14, lineHeight: 1.6, flexShrink: 0 }}>•</span><span>{trimmed.replace(/^[-\*•]\s/, "")}</span></div>;
//         return <p key={i} style={{ marginBottom: 4 }}>{trimmed}</p>;
//       })}
//     </div>
//   );
// }
...
//       <div className="flex flex-wrap items-center justify-between gap-3">
//         <div className="flex items-center gap-2">
//           <div className="p-2 rounded-lg bg-primary/10 border border-primary/20"><Cpu className="w-5 h-5 text-primary" /></div>
//           <span className="text-base font-semibold text-foreground">AutoML Results</span>
//         </div>
...
//       {automlResult.suggestions?.length > 0 && <div className="bg-card border border-border rounded-xl p-5"><h2 className="text-sm font-semibold mb-3 text-primary">Next Steps</h2><ul className="space-y-2">{automlResult.suggestions.map((s, i) => (<li key={i} className="flex items-start gap-2 text-xs"><span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary text-[10px] font-semibold">{i + 1}</span><span>{s}</span></li>))}</ul></div>}
//     </div>
//   );
// }

// function PipelineCreatedCard({ pipeline, allJobs }: { pipeline: PipelineCreatedResult; allJobs: PipelineJob[] }) {
//   const resolveJobName = (jobId: string) => { if (pipeline.job_names?.[jobId]) return pipeline.job_names[jobId]; const found = allJobs.find((j) => j.job_id === jobId); return found ? found.job_name : jobId.slice(0, 8) + "…"; };
//   const scheduleLabel = () => { if (!pipeline.schedule) return null; const { type, hour, minute, day } = pipeline.schedule; const t = `${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}`; if (type === "daily") return `Every day at ${t}`; if (type === "weekly") return `Every week at ${t}`; if (type === "monthly") return `Every month on day ${day} at ${t}`; return `${type} at ${t}`; };
//   const schedStr = scheduleLabel();
//   return (
//     <div style={{ marginTop: 10, background: "linear-gradient(135deg, hsl(142 72% 42% / 0.07), hsl(197 100% 55% / 0.05))", border: "1.5px solid hsl(142 72% 42% / 0.35)", borderRadius: 14, padding: "16px 18px", maxWidth: 480 }}>
//       <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
//         <div style={{ width: 34, height: 34, borderRadius: 10, background: "hsl(142 72% 42%)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><CheckCircle style={{ width: 18, height: 18, color: "#fff" }} /></div>
//         <div><div style={{ fontWeight: 700, fontSize: 14 }}>Pipeline Created Successfully</div><div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", marginTop: 1 }}>{schedStr ? "Scheduled & ready" : "Ready to run on demand"}</div></div>
//       </div>
//       <div style={{ display: "flex", alignItems: "center", gap: 8, background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 9, padding: "8px 12px", marginBottom: 10 }}>
//         <GitBranch style={{ width: 14, height: 14, color: "hsl(var(--primary))", flexShrink: 0 }} />
//         <div><div style={{ fontSize: 10, color: "hsl(var(--muted-foreground))", fontWeight: 600, marginBottom: 1 }}>PIPELINE NAME</div><div style={{ fontSize: 13, fontWeight: 700 }}>{pipeline.name}</div></div>
//       </div>
//       <div style={{ marginBottom: 10 }}>
//         <div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", fontWeight: 600, marginBottom: 6 }}>JOBS ({pipeline.jobs.length})</div>
//         {pipeline.jobs.map((jobId, idx) => (
//           <div key={jobId} style={{ display: "flex", alignItems: "center", gap: 8, background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8, padding: "6px 10px", marginBottom: 4 }}>
//             <span style={{ fontSize: 9, fontWeight: 700, color: "hsl(var(--primary))", background: "hsl(var(--primary) / 0.12)", borderRadius: 4, padding: "2px 6px" }}>{idx + 1}</span>
//             <div><div style={{ fontSize: 12, fontWeight: 600 }}>{resolveJobName(jobId)}</div><div style={{ fontSize: 10, color: "hsl(var(--muted-foreground))", fontFamily: "monospace" }}>{jobId.slice(0, 8)}…</div></div>
//           </div>
//         ))}
//       </div>
//       {schedStr && <div style={{ display: "flex", alignItems: "center", gap: 8, background: "hsl(197 100% 50% / 0.07)", border: "1px solid hsl(197 100% 50% / 0.25)", borderRadius: 9, padding: "8px 12px" }}><Calendar style={{ width: 14, height: 14, color: "hsl(197 100% 38%)", flexShrink: 0 }} /><div><div style={{ fontSize: 10, color: "hsl(var(--muted-foreground))", fontWeight: 600, marginBottom: 1 }}>SCHEDULE</div><div style={{ fontSize: 12, fontWeight: 600, color: "hsl(197 100% 35%)" }}>{schedStr}</div></div><div style={{ marginLeft: "auto" }}><span style={{ fontSize: 10, fontWeight: 600, color: "#fff", background: "hsl(142 72% 38%)", borderRadius: 20, padding: "2px 8px" }}>Active</span></div></div>}
//       <div style={{ marginTop: 10, fontSize: 10, color: "hsl(var(--muted-foreground))", fontFamily: "monospace" }}>ID: {pipeline.pipeline_id}</div>
//     </div>
//   );
// }
...
//         <button onClick={() => navigate("/workflow/powerbi-flow")} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600, color: "#fff", background: "linear-gradient(135deg, #2563eb, #1d4ed8)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, padding: "7px 14px", cursor: "pointer" }}><Share2 style={{ width: 13, height: 13 }} />Deploy to Power BI</button>
//       </div>
//       <div className="flex flex-wrap items-center gap-3 text-xs">
//         <span className="px-3 py-1.5 rounded-full bg-primary/10 text-primary font-medium">{kpiVisuals.length} KPIs</span>
//         <span className="px-3 py-1.5 rounded-full bg-amber-500/10 text-amber-400 font-medium">{dashboard.total_visuals} Visuals</span>
...
//       {kpiVisuals.length > 0 && (
//         <div>
//           <div className="flex items-center gap-2 mb-3"><TrendingUp className="w-4 h-4 text-primary" /><span className="text-sm font-semibold text-foreground">Key Results</span></div>
//           <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
//             {kpiVisuals.map((kpi: any, i: number) => {
//               const val = kpi.value;
//               const formatted = val == null ? "No data" : val >= 1_000_000 ? `${(val / 1_000_000).toFixed(2)}M` : val >= 1_000 ? `${(val / 1_000).toFixed(2)}K` : val.toLocaleString(undefined, { maximumFractionDigits: 2 });
//               return <div key={i} className="bg-card border border-border rounded-xl p-4"><p className="text-xs text-muted-foreground font-medium mb-1">{kpi.chart_name}</p><p className="text-xl font-bold text-primary mb-1">{formatted}</p><p className="text-[10px] text-muted-foreground">{kpi.description || "Result from query"}</p></div>;
//             })}
//           </div>
//         </div>
//       )}
...
//   return (
//     <div style={{ marginTop: 10, maxWidth: 420 }}>
//       <p style={{ fontSize: 12, fontWeight: 600, color: "hsl(var(--muted-foreground))", marginBottom: 8 }}>Select jobs to include in the pipeline:</p>
//       <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 240, overflowY: "auto" }}>
//         {jobs.map((job) => (
//           <label key={job.job_id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 10px", borderRadius: 8, cursor: "pointer", border: selected.has(job.job_id) ? "1.5px solid hsl(var(--primary))" : "1.5px solid hsl(var(--border))", background: selected.has(job.job_id) ? "hsl(var(--primary) / 0.08)" : "hsl(var(--card))", transition: "all 0.15s ease" }}>
//             <input type="checkbox" checked={selected.has(job.job_id)} onChange={() => toggle(job.job_id)} style={{ accentColor: "hsl(var(--primary))", width: 14, height: 14 }} />
//             <div><span style={{ fontSize: 12, fontWeight: 600 }}>{job.job_name}</span><span style={{ fontSize: 10, color: "hsl(var(--muted-foreground))", marginLeft: 6 }}>{job.job_id.slice(0, 8)}…</span></div>
//           </label>
//         ))}
//       </div>
//       <button disabled={selected.size === 0} onClick={() => onConfirm(Array.from(selected))} style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600, color: "#fff", background: selected.size === 0 ? "hsl(var(--muted))" : "linear-gradient(135deg, hsl(267 84% 60%), hsl(220 90% 60%))", border: "none", borderRadius: 8, padding: "7px 16px", cursor: selected.size === 0 ? "not-allowed" : "pointer", opacity: selected.size === 0 ? 0.5 : 1 }}>
//         <Check style={{ width: 13, height: 13 }} />Confirm Selection ({selected.size})
//       </button>
//     </div>
//   );
// }
...
//   const [saving, setSaving] = useState(false);
//   const [saveStatus, setSaveStatus] = useState<"idle" | "success" | "error">("idle");
//   const handleSaveJobName = async (newName: string) => { const res = await fetch(`${BASE_URL}/rename-job`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, job_id: result.job_id, job_name: newName }) }); const data = await res.json(); if (data.status === "success") setJobName(data.job_name || newName); else throw new Error("Failed"); };
//   const handleRenameDataset = async (newName: string) => { const res = await fetch(`${BASE_URL}/rename-dataset`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, job_id: result.job_id, old_name: datasetName, new_name: newName }) }); const data = await res.json(); if (data.status === "success") setDatasetName(data.new_name || newName); else throw new Error("Failed"); };
//   const handleSaveJob = async () => {
//     if (saving || saveStatus === "success") return;
//     setSaving(true);
//     const aivolveUser = getAivolveUser();
//     try {
//       const res = await fetch(`${BASE_URL}/save-job`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, job_id: result.job_id, dataset: localStorage.getItem("current_dataset_name") || "", onelake_path: localStorage.getItem("current_onelake_path") || "", session_id: aivolveUser?.session_id || "", user_email: aivolveUser?.email || "" }) });
//       const data = await res.json();
//       if (data.status !== "success") { setSaveStatus("error"); setTimeout(() => setSaveStatus("idle"), 3000); return; }
//       setSaveStatus("success");
//     } catch { setSaveStatus("error"); setTimeout(() => setSaveStatus("idle"), 3000); } finally { setSaving(false); }
//   };
//   return (
//     <div className="mt-3 w-full max-w-2xl bg-card border border-border rounded-xl p-4 shadow-sm space-y-3">
//       <EditableField label="Job Name" value={jobName} onSave={handleSaveJobName} />
...
//           <p className="text-xs font-semibold text-foreground mb-2">Data Model</p>
//           <div className="h-[400px] w-full border border-border rounded-lg overflow-hidden">
//             <ReactFlow nodes={buildStarSchema(result.data_model, result.relationships, result.schemas).nodes} edges={buildStarSchema(result.data_model, result.relationships, result.schemas).edges} nodeTypes={schemaNodeTypes} edgeTypes={schemaEdgeTypes} fitView fitViewOptions={{ padding: 0.3 }} proOptions={{ hideAttribution: true }}><Background gap={20} size={1} /><Controls showInteractive={false} /></ReactFlow>
//           </div>
//           <DataModelSummary dataModel={result.data_model} relationships={result.relationships} schemas={result.schemas} />
//         </div>
//       )}
//       {result.final_dataset && <EditableField label="Dataset" value={datasetName} onSave={handleRenameDataset} />}
//       {result.final_dataset?.preview && (
//         <div className="overflow-auto border border-border rounded-lg">
//           <table className="text-xs w-full">
...
//         <button onClick={handleSaveJob} disabled={saving || saveStatus === "success"} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600, color: saveStatus === "success" ? "#fff" : saveStatus === "error" ? "#fff" : "hsl(var(--foreground))", background: saveStatus === "success" ? "hsl(142 72% 38%)" : saveStatus === "error" ? "hsl(0 72% 51%)" : "hsl(var(--muted))", border: "1.5px solid hsl(var(--border))", borderRadius: 8, padding: "7px 14px", cursor: "pointer" }}>
//           {saving ? <Loader2 style={{ width: 13, height: 13 }} className="animate-spin" /> : saveStatus === "success" ? <Check style={{ width: 13, height: 13 }} /> : <span>💾</span>}
//           {saving ? "Saving…" : saveStatus === "success" ? "Saved!" : "Save Dataset"}
//         </button>
//         {result.download_url && <button onClick={() => onDownload(result.download_url)} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600, color: "#fff", background: "linear-gradient(135deg, hsl(267 84% 60%), hsl(220 90% 60%))", border: "none", borderRadius: 8, padding: "7px 14px", cursor: "pointer" }}><span>⬇️</span> Download CSV</button>}
//       </div>
//     </div>
//   );
// }
...
// // ─────────────────────────────────────────────────────────────
// // DatasetSidePanel — right-side drawer
// // ─────────────────────────────────────────────────────────────
// function DatasetSidePanel({
//   open,
//   onClose,
//   userId,
//   activeDataset,
...
//           jobName: item.job_name || "Unnamed Job",
//           datasetName: item.dataset_name || "Unnamed Dataset",
//           lastRun: item.completed_at ? new Date(item.completed_at).toLocaleString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "—",
//           completedAt: item.completed_at || "",
//           job_id: item.job_id || "",
//           dataset_path: item.dataset_path || item.blob_path || "",
//           onelake_path: item.onelake_path || "",
...
//   const handleSelect = (ds: DatasetItem) => {
//     // Build paths explicitly from userId + job_id + datasetName
//     const datasetPath = `${userId}/${ds.job_id}/${ds.datasetName}`;
//     const onelakePath = `Files/Datasets/${userId}/${ds.job_id}/${ds.datasetName}`;
//     onSelect({
//       datasetName: ds.datasetName,
//       jobName: ds.jobName,
//       job_id: ds.job_id,
...
//         <div
//           onClick={onClose}
//           style={{ position: "fixed", inset: 0, zIndex: 40, background: "rgba(0,0,0,0.35)", backdropFilter: "blur(2px)", transition: "opacity 0.2s" }}
//         />
//       )}
...
//       }}>
//         {/* Header */}
//         <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid hsl(var(--border))", background: "hsl(var(--card))", flexShrink: 0 }}>
//           <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
//             <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
//               <div style={{ width: 34, height: 34, borderRadius: 10, background: "linear-gradient(135deg, hsl(267 84% 60%), hsl(220 90% 60%))", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
//                 <Database style={{ width: 16, height: 16, color: "#fff" }} />
//               </div>
//               <div>
//                 <div style={{ fontWeight: 700, fontSize: 15, color: "hsl(var(--foreground))" }}>Your Datasets</div>
//                 <div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", marginTop: 1 }}>
//                   {loading ? "Loading…" : `${datasets.length} dataset${datasets.length !== 1 ? "s" : ""} available`}
//                 </div>
//               </div>
...
//             <button
//               onClick={onClose}
//               style={{ width: 30, height: 30, borderRadius: 8, background: "hsl(var(--muted))", border: "1px solid hsl(var(--border))", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "hsl(var(--muted-foreground))", transition: "all 0.15s ease" }}
...
//               onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "hsl(var(--muted))"; (e.currentTarget as HTMLButtonElement).style.color = "hsl(var(--muted-foreground))"; }}
//             >
...
//           {/* Active badge */}
//           {activeDataset && (
//             <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 6, background: "hsl(267 84% 60% / 0.08)", border: "1px solid hsl(267 84% 60% / 0.25)", borderRadius: 8, padding: "6px 10px" }}>
//               <div style={{ width: 7, height: 7, borderRadius: "50%", background: "hsl(142 72% 42%)", flexShrink: 0 }} />
//               <span style={{ fontSize: 11, color: "hsl(var(--muted-foreground))" }}>Active:</span>
//               <span style={{ fontSize: 11, fontWeight: 700, color: "hsl(var(--primary))", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{activeDataset.datasetName}</span>
//             </div>
//           )}
//         </div>

//         {/* Search */}
//         <div style={{ padding: "12px 16px", borderBottom: "1px solid hsl(var(--border) / 0.5)", flexShrink: 0 }}>
//           <div style={{ position: "relative" }}>
...
//               onChange={(e) => setSearch(e.target.value)}
//               placeholder="Search datasets…"
//               style={{ width: "100%", boxSizing: "border-box", paddingLeft: 30, paddingRight: search ? 28 : 10, paddingTop: 8, paddingBottom: 8, fontSize: 12, background: "hsl(var(--muted) / 0.5)", border: "1px solid hsl(var(--border))", borderRadius: 8, color: "hsl(var(--foreground))", outline: "none" }}
//               onFocus={(e) => (e.currentTarget.style.borderColor = "hsl(var(--primary) / 0.5)")}
//               onBlur={(e) => (e.currentTarget.style.borderColor = "hsl(var(--border))")}
//             />
//             {search && (
//               <button onClick={() => setSearch("")} style={{ position: "absolute", right: 7, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: 2, display: "flex", alignItems: "center", color: "hsl(var(--muted-foreground))" }}>
//                 <X style={{ width: 11, height: 11 }} />
//               </button>
//             )}
//           </div>
...
//           {loading ? (
//             <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 200, gap: 10, color: "hsl(var(--muted-foreground))" }}>
//               <Loader2 style={{ width: 24, height: 24 }} className="animate-spin" />
//               <span style={{ fontSize: 12 }}>Loading your datasets…</span>
//             </div>
//           ) : filtered.length === 0 ? (
//             <div style={{ textAlign: "center", padding: "40px 16px" }}>
//               <Database style={{ width: 32, height: 32, color: "hsl(var(--muted-foreground))", margin: "0 auto 10px" }} />
//               <div style={{ fontSize: 13, fontWeight: 600, color: "hsl(var(--foreground))", marginBottom: 4 }}>{search ? "No matches found" : "No datasets yet"}</div>
//               <div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))" }}>{search ? "Try a different term" : "Run a pipeline to generate datasets"}</div>
//             </div>
//           ) : (
//             filtered.map((ds, i) => {
//               const isActive = activeDataset?.datasetName === ds.datasetName && activeDataset?.job_id === ds.job_id;
//               return (
//                 <div key={i} style={{ border: isActive ? "1.5px solid hsl(var(--primary))" : "1px solid hsl(var(--border))", borderRadius: 12, marginBottom: 8, background: isActive ? "hsl(267 84% 60% / 0.06)" : "hsl(var(--card))", transition: "all 0.15s ease", overflow: "hidden" }}
//                   onMouseEnter={(e) => { if (!isActive) (e.currentTarget as HTMLDivElement).style.borderColor = "hsl(var(--primary) / 0.35)"; }}
//                   onMouseLeave={(e) => { if (!isActive) (e.currentTarget as HTMLDivElement).style.borderColor = "hsl(var(--border))"; }}
//                 >
//                   {/* Card content */}
//                   <div style={{ padding: "12px 14px" }}>
//                     <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
//                       {/* Icon */}
//                       <div style={{ width: 36, height: 36, borderRadius: 9, flexShrink: 0, background: isActive ? "linear-gradient(135deg, hsl(267 84% 60%), hsl(220 90% 60%))" : "hsl(var(--muted))", display: "flex", alignItems: "center", justifyContent: "center" }}>
//                         <Table2 style={{ width: 16, height: 16, color: isActive ? "#fff" : "hsl(var(--muted-foreground))" }} />
//                       </div>

//                       {/* Info */}
//                       <div style={{ flex: 1, minWidth: 0 }}>
//                         <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
//                           <span style={{ fontWeight: 700, fontSize: 13, color: isActive ? "hsl(var(--primary))" : "hsl(var(--foreground))", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 170 }}>{ds.datasetName}</span>
//                           {isActive && <span style={{ fontSize: 9, fontWeight: 700, color: "#fff", background: "hsl(var(--primary))", borderRadius: 4, padding: "1px 6px", flexShrink: 0 }}>ACTIVE</span>}
//                         </div>
//                         <div style={{ fontSize: 11, color: "hsl(var(--muted-foreground))", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{ds.jobName}</div>
//                         <div style={{ fontSize: 10, color: "hsl(var(--muted-foreground))", marginTop: 3, display: "flex", alignItems: "center", gap: 4 }}>
...
//                   {/* Action row */}
//                   <div style={{ display: "flex", borderTop: "1px solid hsl(var(--border) / 0.6)", background: "hsl(var(--muted) / 0.3)" }}>
//                     <button
//                       onClick={() => openPreview(ds)}
//                       style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "8px 0", fontSize: 11, fontWeight: 600, color: "hsl(var(--muted-foreground))", background: "none", border: "none", borderRight: "1px solid hsl(var(--border) / 0.6)", cursor: "pointer", transition: "all 0.15s ease" }}
//                       onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.color = "hsl(197 100% 38%)"; (e.currentTarget as HTMLButtonElement).style.background = "hsl(197 100% 50% / 0.08)"; }}
//                       onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.color = "hsl(var(--muted-foreground))"; (e.currentTarget as HTMLButtonElement).style.background = "none"; }}
//                     >
//                       <Eye style={{ width: 12, height: 12 }} /> Preview
//                     </button>

//                     {isActive ? (
//                       <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "8px 0", fontSize: 11, fontWeight: 600, color: "hsl(142 72% 38%)" }}>
//                         <CheckCircle style={{ width: 12, height: 12 }} /> Selected
//                       </div>
//                     ) : (
//                       <button
//                         onClick={() => handleSelect(ds)}
//                         style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "8px 0", fontSize: 11, fontWeight: 600, color: "hsl(var(--primary))", background: "none", border: "none", cursor: "pointer", transition: "all 0.15s ease" }}
//                         onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.color = "#fff"; (e.currentTarget as HTMLButtonElement).style.background = "hsl(var(--primary))"; }}
//                         onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.color = "hsl(var(--primary))"; (e.currentTarget as HTMLButtonElement).style.background = "none"; }}
//                       >
//                         <PlayCircle style={{ width: 12, height: 12 }} /> Use this
//                       </button>
//                     )}
//                   </div>
//                 </div>
...
//         {/* Footer hint */}
//         {!loading && filtered.length > 0 && (
//           <div style={{ padding: "10px 16px", borderTop: "1px solid hsl(var(--border) / 0.5)", background: "hsl(var(--card))", flexShrink: 0 }}>
//             <p style={{ fontSize: 10, color: "hsl(var(--muted-foreground))", textAlign: "center", lineHeight: 1.5 }}>
//               Select a dataset to use it for analysis, dashboards & AutoML
//             </p>
//           </div>
//         )}
//       </div>
...
//       {previewTarget && createPortal(
//         <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
//           <div onClick={() => { setPreviewTarget(null); setPreviewData(null); setPreviewError(null); }} style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.65)", backdropFilter: "blur(4px)" }} />
//           <div style={{ position: "relative", background: "hsl(var(--background))", border: "1px solid hsl(var(--border))", borderRadius: 16, width: "90vw", maxWidth: 920, maxHeight: "82vh", display: "flex", flexDirection: "column", boxShadow: "0 32px 80px rgba(0,0,0,0.5)", overflow: "hidden" }}>
//             <div style={{ padding: "16px 20px", borderBottom: "1px solid hsl(var(--border))", display: "flex", alignItems: "center", justifyContent: "space-between", background: "hsl(var(--card))", flexShrink: 0 }}>
//               <div>
//                 <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 3 }}>Dataset Preview</div>
//                 <div style={{ fontSize: 12, color: "hsl(var(--muted-foreground))", display: "flex", alignItems: "center", gap: 6 }}>
//                   <span style={{ color: "hsl(var(--primary))", fontWeight: 600, background: "hsl(var(--primary) / 0.1)", border: "1px solid hsl(var(--primary) / 0.2)", borderRadius: 5, padding: "1px 7px", fontSize: 11 }}>{previewTarget.datasetName}</span>
//                   {previewData && <span>{previewData.total_columns} columns × {previewData.total_rows} rows</span>}
//                 </div>
//               </div>
//               <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
//                 {activeDataset?.datasetName !== previewTarget.datasetName && (
//                   <button onClick={() => { handleSelect(previewTarget); setPreviewTarget(null); setPreviewData(null); setPreviewError(null); }} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600, color: "#fff", background: "linear-gradient(135deg, hsl(267 84% 60%), hsl(220 90% 60%))", border: "none", borderRadius: 8, padding: "7px 14px", cursor: "pointer" }}>
//                     <PlayCircle style={{ width: 13, height: 13 }} />Use this Dataset
//                   </button>
//                 )}
//                 <button onClick={() => { setPreviewTarget(null); setPreviewData(null); setPreviewError(null); }} style={{ background: "hsl(var(--muted))", border: "1px solid hsl(var(--border))", borderRadius: 8, padding: "6px 8px", cursor: "pointer", display: "flex", alignItems: "center" }}>
//                   <X style={{ width: 14, height: 14, color: "hsl(var(--muted-foreground))" }} />
//                 </button>
//               </div>
//             </div>
//             <div style={{ flex: 1, overflow: "auto" }}>
//               {previewLoading ? (
//                 <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 240, gap: 12, color: "hsl(var(--muted-foreground))", fontSize: 13 }}><Loader2 style={{ width: 24, height: 24 }} className="animate-spin" />Loading preview…</div>
//               ) : previewError ? (
//                 <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 240, gap: 10 }}><AlertCircle style={{ width: 28, height: 28, color: "hsl(0 72% 51%)" }} /><div style={{ fontSize: 13, fontWeight: 600, color: "hsl(0 72% 51%)" }}>Failed to load preview</div><div style={{ fontSize: 12, color: "hsl(var(--muted-foreground))" }}>{previewError}</div></div>
//               ) : previewData ? (
//                 <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
//                   <thead style={{ position: "sticky", top: 0, zIndex: 1 }}>
//                     <tr style={{ background: "hsl(var(--primary))" }}>
//                       {previewData.columns.map((col) => <th key={col} style={{ padding: "10px 14px", textAlign: "left", fontWeight: 600, color: "#fff", whiteSpace: "nowrap", borderRight: "1px solid rgba(255,255,255,0.1)" }}><div style={{ fontSize: 12 }}>{col}</div><div style={{ fontSize: 10, opacity: 0.75, fontWeight: 400, marginTop: 1 }}>{previewData.column_types[col] || "?"}</div></th>)}
//                     </tr>
//                   </thead>
//                   <tbody>
//                     {previewData.preview_rows.length === 0 ? (
//                       <tr><td colSpan={previewData.columns.length} style={{ padding: "40px 16px", textAlign: "center", color: "hsl(var(--muted-foreground))", fontSize: 13 }}>No preview rows available</td></tr>
//                     ) : (
//                       previewData.preview_rows.map((row, ri) => (
//                         <tr key={ri} style={{ borderBottom: "1px solid hsl(var(--border) / 0.5)", background: ri % 2 === 0 ? "transparent" : "hsl(var(--muted) / 0.3)" }}
//                           onMouseEnter={(e) => (e.currentTarget.style.background = "hsl(var(--primary) / 0.04)")}
//                           onMouseLeave={(e) => (e.currentTarget.style.background = ri % 2 === 0 ? "transparent" : "hsl(var(--muted) / 0.3)")}
//                         >
//                           {previewData.columns.map((col) => <td key={col} style={{ padding: "8px 14px", color: "hsl(var(--foreground))", whiteSpace: "nowrap", borderRight: "1px solid hsl(var(--border) / 0.3)" }}>{row[col] != null ? String(row[col]) : <span style={{ color: "hsl(var(--muted-foreground))" }}>—</span>}</td>)}
//                         </tr>
//                       ))
//                     )}
//                   </tbody>
...
// // ─────────────────────────────────────────────────────────────
// // buildMessagesFromThreads
// // ─────────────────────────────────────────────────────────────
// function buildMessagesFromThreads(threadsData: any): Message[] {
//   const msgs: Message[] = [];
//   const actions: any[] = threadsData?.actions || [];
//   for (const action of actions) {
//     const { type, prompt, response, timestamp: ts } = action;
//     const timestamp = new Date(ts || Date.now());
//     if (prompt) msgs.push({ id: `${type}-user-${timestamp.getTime()}`, role: "user", content: prompt, timestamp });
//     if (type === "etl" && response?.status === "success") {
//       msgs.push({ id: `${type}-assistant-${timestamp.getTime()}`, role: "assistant", content: response.message || "Job completed.", result: { pipeline_name: response.pipeline_name || "", suggested_job_name: response.suggested_job_name || response.pipeline_name || "", job_name: action.job_name || response.job_name || "", job_id: threadsData.job_id || "", data_model: response.data_model, relationships: response.relationships, schemas: response.schemas, final_dataset: response.final_dataset, download_url: action.download_url || "" }, timestamp });
//     } else if (type === "powerbi" && response?.status === "success") {
//       msgs.push({ id: `${type}-assistant-${timestamp.getTime()}`, role: "assistant", content: "Power BI dashboard generated!", dashboardResult: response, timestamp });
//     } else if (type === "automl" && response?.status === "success") {
//       msgs.push({ id: `${type}-assistant-${timestamp.getTime()}`, role: "assistant", content: "AutoML completed!", automlResult: response, timestamp });
//     } else if (type === "file_upload" && response) {
//       msgs.push({ id: `${type}-assistant-${timestamp.getTime()}`, role: "assistant", content: response.response || "File uploaded.", timestamp });
//     }
//   }
//   return msgs;
// }
...
//   useEffect(() => {
//     if (!userId || !jobId) return;
//     fetch(`${BASE_URL}/threads?user_id=${userId}&job_id=${jobId}`)
//       .then((r) => r.json())
//       .then((data) => { const restored = buildMessagesFromThreads(data); if (restored.length > 0) setMessages(restored); })
//       .catch(console.error);
//   }, []);

//   const handleDatasetSelect = (ds: ActiveDataset) => {
//     setActiveDataset(ds);
//     // Store all three keys with correctly constructed paths
//     localStorage.setItem("current_dataset_name", ds.datasetName);
//     localStorage.setItem("current_dataset_path", ds.dataset_path);
//     localStorage.setItem("current_onelake_path", ds.onelake_path || `Files/Datasets/${ds.dataset_path}`);
//     setDatasetPanelOpen(false);
//     setMessages((prev) => [...prev, { id: Date.now().toString(), role: "assistant", content: `✅ **${ds.datasetName}** is now your active dataset. I'll use it for analysis, dashboards, and AutoML. What would you like to do with it?`, timestamp: new Date() }]);
//   };
...
//   const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (file) setAttachedFile(file); e.target.value = ""; };
//   const removeAttachment = () => setAttachedFile(null);
...
//   const isAutoMLIntent = (t: string) => { const l = t.toLowerCase(); return l.includes("build model") || l.includes("train model") || l.includes("ml model") || l.includes("machine learning") || l.includes("automl") || l.includes("regression") || l.includes("classification") || l.includes("predict"); };
...
//     setLoading(true);
//     try {
//       const res = await fetch(`${BASE_URL}/chat`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, job_id: jobId, message: content, selected_jobs: [] }) });
//       const data = await res.json();
//       const jobs: PipelineJob[] = data.jobs || [];
//       if (jobs.length > 0) setAllKnownJobs(jobs);
//       setMessages((prev) => [...prev, { id: Date.now().toString(), role: "assistant", content: data.response || "Select jobs for pipeline:", pipelineJobs: jobs, timestamp: new Date() }]);
//       setPipelineStep("awaiting_job_selection");
//     } catch { setMessages((prev) => [...prev, { id: Date.now().toString(), role: "assistant", content: "Failed to start pipeline creation.", error: true, timestamp: new Date() }]); setPipelineStep("idle"); } finally { setLoading(false); }
//   };
...
//     setLoading(true);
//     try {
//       const res = await fetch(`${BASE_URL}/chat`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, job_id: jobId, message: "selected", selected_jobs: selectedIds }) });
//       const data = await res.json();
//       setMessages((prev) => [...prev, { id: Date.now().toString(), role: "assistant", content: data.response || "Please enter a pipeline name.", timestamp: new Date() }]);
//       setPipelineStep("awaiting_pipeline_name");
//     } catch { setMessages((prev) => [...prev, { id: Date.now().toString(), role: "assistant", content: "Failed to confirm job selection.", error: true, timestamp: new Date() }]); setPipelineStep("idle"); } finally { setLoading(false); }
//   };

//   const handlePipelineNameSubmit = async (content: string) => {
//     setLoading(true);
//     try {
//       const res = await fetch(`${BASE_URL}/chat`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, job_id: jobId, message: content, selected_jobs: pipelineSelectedJobs }) });
//       const data = await res.json();
//       const jobNamesMap: Record<string, string> = {};
//       allKnownJobs.forEach((j) => { jobNamesMap[j.job_id] = j.job_name; });
//       const pipelineCreated: PipelineCreatedResult | undefined = data.pipeline ? { ...data.pipeline, job_names: jobNamesMap } : undefined;
//       setMessages((prev) => [...prev, { id: Date.now().toString(), role: "assistant", content: data.response || "Pipeline created!", pipelineCreated, timestamp: new Date() }]);
//       setPipelineStep("idle");
//       setPipelineSelectedJobs([]);
//     } catch { setMessages((prev) => [...prev, { id: Date.now().toString(), role: "assistant", content: "Failed to create pipeline.", error: true, timestamp: new Date() }]); setPipelineStep("idle"); } finally { setLoading(false); }
//   };

//   const sendMessage = async (text?: string) => {
//     const content = (text || input).trim();
//     if ((!content && !attachedFile) || loading) return;
//     const userMsg: Message = { id: Date.now().toString(), role: "user", content: content || `Uploaded file: ${attachedFile?.name}`, attachment: attachedFile?.name, timestamp: new Date() };
//     setMessages((prev) => [...prev, userMsg]);
//     setInput("");
//     setLoading(true);
//     removeAttachment();

//     if (pipelineStep === "awaiting_pipeline_name") { setLoading(false); await handlePipelineNameSubmit(content); return; }
//     if (isGreeting(content)) { setLoading(false); setTimeout(() => setMessages((prev) => [...prev, { id: (Date.now() + 1).toString(), role: "assistant", content: "Hello! 👋 How can I help you with your data today?", timestamp: new Date() }]), 600); return; }
//     if (isDatasetIntent(content)) { setLoading(false); setDatasetPanelOpen(true); return; }
//     if (isCreatePipelineIntent(content)) { setLoading(false); await handleCreatePipeline(content); return; }

//     if (isAutoMLIntent(content)) {
//       try {
//         const aivolveUser = getAivolveUser();
//         const res = await fetch(`${BASE_URL}/automl/run`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, job_id: jobId, session_id: aivolveUser?.session_id || "", user_email: aivolveUser?.email || "", query: content }) });
//         const data = await res.json();
...
//       } catch { setMessages((prev) => [...prev, { id: (Date.now() + 1).toString(), role: "assistant", content: "Failed to run AutoML.", error: true, timestamp: new Date() }]); } finally { setLoading(false); }
//       return;
//     }
...
//         const res = await fetch(`${BASE_URL}/generate_powerbi_dashboard`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ csv_blob: csvBlob, user_prompt: content, user_id: userId, job_id: jobId }) });
//         const data = await res.json();
...
//       } catch { setMessages((prev) => [...prev, { id: (Date.now() + 1).toString(), role: "assistant", content: "Failed to generate the dashboard.", error: true, timestamp: new Date() }]); } finally { setLoading(false); }
//       return;
//     }

//     // Standard ETL
//     try {
//       const res = await fetch(`${BASE_URL}/run`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, job_id: jobId, prompt: content }) });
//       const data = await res.json();
//       if (data.final_dataset?.dataset_path) localStorage.setItem("current_dataset_path", data.final_dataset.dataset_path);
//       if (data.final_dataset?.onelake_path) localStorage.setItem("current_onelake_path", data.final_dataset.onelake_path);
//       if (data.final_dataset?.dataset_name) localStorage.setItem("current_dataset_name", data.final_dataset.dataset_name);
//       if (data.final_dataset?.dataset_path) setActiveDataset({ datasetName: data.final_dataset.dataset_name || "", jobName: data.suggested_job_name || data.pipeline_name || "", job_id: data.job_id || "", dataset_path: data.final_dataset.dataset_path, onelake_path: data.final_dataset.onelake_path });
//       setMessages((prev) => [...prev, { id: (Date.now() + 1).toString(), role: "assistant", content: data.message || "Job completed.", result: { pipeline_name: data.pipeline_name, suggested_job_name: data.suggested_job_name || data.pipeline_name, job_name: data.job_name || "", job_id: data.job_id, data_model: data.data_model, relationships: data.relationships, schemas: data.schemas, final_dataset: data.final_dataset, download_url: data.download_url }, timestamp: new Date() }]);
//     } catch { setMessages((prev) => [...prev, { id: (Date.now() + 1).toString(), role: "assistant", content: "I couldn't reach the server. Please try again.", error: true, timestamp: new Date() }]); } finally { setLoading(false); }
//   };
...
//   return (
//     <WorkflowLayout>
//       <div className="flex flex-col h-screen bg-background" style={{ position: "relative" }}>

//         {/* Dataset Side Panel */}
//         <DatasetSidePanel
//           open={datasetPanelOpen}
...
//         {/* ── Header ── */}
//         <header className="sticky top-0 z-20 flex items-center justify-between px-6 py-3.5 bg-background/90 backdrop-blur-md border-b border-border">
//           <div className="flex items-center gap-3">
...
//                 border: `1.5px solid ${datasetPanelOpen || activeDataset ? "hsl(var(--primary) / 0.5)" : "hsl(var(--border))"}`,
//                 background: datasetPanelOpen ? "hsl(var(--primary) / 0.1)" : activeDataset ? "hsl(267 84% 60% / 0.08)" : "hsl(var(--muted))",
//                 color: (datasetPanelOpen || activeDataset) ? "hsl(var(--primary))" : "hsl(var(--muted-foreground))",
//                 cursor: "pointer", transition: "all 0.15s ease",
//               }}
//             >
//               <Database style={{ width: 13, height: 13 }} />
...
//                 </span>
//               ) : "Datasets"}
//               {activeDataset && <div style={{ width: 6, height: 6, borderRadius: "50%", background: "hsl(142 72% 42%)", flexShrink: 0 }} />}
//               <ChevronRight style={{ width: 11, height: 11, transform: datasetPanelOpen ? "rotate(180deg)" : "none", transition: "transform 0.2s" }} />
//             </button>

//             {/* Clear */}
//             {messages.length > 0 && (
//               <button
//                 onClick={() => { setMessages([]); setPipelineStep("idle"); setPipelineSelectedJobs([]); }}
//                 style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 500, color: "hsl(var(--muted-foreground))", background: "hsl(var(--muted))", border: "1px solid hsl(var(--border))", borderRadius: 8, padding: "6px 10px", cursor: "pointer", transition: "all 0.15s ease" }}
//               >
...
//                 <div className="flex flex-wrap gap-2 justify-center">
//                   {[
//                     { icon: "🗂️", label: "Select a dataset", action: () => setDatasetPanelOpen(true) },
//                     { icon: "📊", label: "Build a dashboard", action: () => setInput("Generate a dashboard") },
//                     { icon: "🤖", label: "Train an ML model", action: () => setInput("Build a machine learning model") },
//                     { icon: "🔄", label: "Create a pipeline", action: () => setInput("Create pipeline") },
//                   ].map((chip) => (
//                     <button key={chip.label} onClick={chip.action}
//                       style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 500, color: "hsl(var(--foreground))", background: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 20, padding: "7px 14px", cursor: "pointer", transition: "all 0.15s ease" }}
//                       onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.borderColor = "hsl(var(--primary) / 0.5)"; (e.currentTarget as HTMLButtonElement).style.background = "hsl(var(--primary) / 0.06)"; }}
...
//                     <div className={`flex flex-col max-w-[78%] ${msg.role === "user" ? "items-end" : "items-start"}`}>
//                       <div className={`px-4 py-2.5 rounded-2xl text-[15px] leading-relaxed whitespace-pre-wrap break-words shadow-sm ${msg.role === "user" ? "bg-gradient-to-br from-purple-600 to-indigo-600 text-white rounded-tr-sm" : msg.error ? "bg-red-500/10 text-red-400 border border-red-500/20 rounded-tl-sm" : "bg-card text-card-foreground border border-border rounded-tl-sm"}`}>
//                         {msg.attachment && msg.role === "user" && <div className="flex items-center gap-2 mb-1.5 pb-1.5 border-b border-white/20 text-xs opacity-90"><FileText className="w-3.5 h-3.5" />{msg.attachment}</div>}
//                         {msg.content}
//                       </div>
//                       {msg.pipelineJobs && msg.pipelineJobs.length > 0 && pipelineStep === "awaiting_job_selection" && <div className="mt-2 w-full max-w-md"><PipelineJobSelector jobs={msg.pipelineJobs} onConfirm={handleJobSelectionConfirm} /></div>}
//                       {msg.result && !msg.error && <ResultCard result={msg.result} userId={userId} onDownload={handleDownload} />}
...
//                 {loading && (
//                   <div className="flex gap-3">
//                     <div className="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 shadow-sm flex-shrink-0"><Bot className="w-4 h-4 text-white" /></div>
//                     <div className="px-4 py-3 bg-card border border-border rounded-2xl rounded-tl-sm shadow-sm">
//                       <div className="flex items-center gap-1">
//                         <span className="w-2 h-2 bg-purple-400 rounded-full animate-bounce [animation-delay:-0.3s]" />
//                         <span className="w-2 h-2 bg-purple-500 rounded-full animate-bounce [animation-delay:-0.15s]" />
...
//         {/* ── Footer / Input ── */}
//         <footer className="sticky bottom-0 z-20 bg-background/90 backdrop-blur-md border-t border-border">
//           <div className="max-w-3xl mx-auto px-4 py-3">
//             {/* Active dataset slim context line */}
//             {activeDataset && (
//               <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6, fontSize: 11, color: "hsl(var(--muted-foreground))" }}>
//                 <div style={{ width: 6, height: 6, borderRadius: "50%", background: "hsl(142 72% 42%)", flexShrink: 0 }} />
//                 <span>Working on:</span>
//                 <span style={{ fontWeight: 700, color: "hsl(var(--primary))" }}>{activeDataset.datasetName}</span>
//                 <button onClick={() => setDatasetPanelOpen(true)} style={{ fontSize: 10, fontWeight: 600, color: "hsl(var(--muted-foreground))", background: "none", border: "none", cursor: "pointer", textDecoration: "underline", textUnderlineOffset: 2 }}>Change</button>
//                 <button onClick={handleClearActiveDataset} style={{ display: "flex", alignItems: "center", marginLeft: 2, background: "none", border: "none", cursor: "pointer", color: "hsl(var(--muted-foreground))", padding: 0 }} title="Remove dataset">
...
//                 <FileText className="w-4 h-4 text-primary flex-shrink-0" />
//                 <span className="text-sm text-foreground truncate flex-1">{attachedFile.name}</span>
//                 <button onClick={removeAttachment} className="p-1 rounded-md hover:bg-primary/10 text-muted-foreground"><X className="w-3.5 h-3.5" /></button>
//               </div>
//             )}

//             <div className="flex items-end gap-2 bg-card border border-border rounded-2xl shadow-sm focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-primary/20 transition-all px-2 py-1.5">
//               <button onClick={() => fileInputRef.current?.click()} className="text-muted-foreground hover:text-primary p-2 rounded-lg hover:bg-muted transition-colors" aria-label="Attach file"><Paperclip className="w-5 h-5" /></button>

//               {/* Datasets quick button inside input */}
//               <button
//                 onClick={() => setDatasetPanelOpen((v) => !v)}
//                 title="Select dataset"
//                 style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 600, flexShrink: 0, padding: "5px 9px", borderRadius: 8, border: "1px solid hsl(var(--border))", background: activeDataset ? "hsl(267 84% 60% / 0.1)" : "hsl(var(--muted))", color: activeDataset ? "hsl(var(--primary))" : "hsl(var(--muted-foreground))", cursor: "pointer", borderColor: activeDataset ? "hsl(var(--primary) / 0.4)" : "hsl(var(--border))", transition: "all 0.15s ease" }}
//               >
//                 <Database style={{ width: 12, height: 12 }} />
//                 {activeDataset ? <span style={{ maxWidth: 80, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{activeDataset.datasetName}</span> : "Datasets"}
//               </button>
...
//                 onChange={(e) => setInput(e.target.value)}
//                 onKeyDown={handleKeyDown}
//                 placeholder={pipelineStep === "awaiting_pipeline_name" ? "Enter pipeline name…" : activeDataset ? `Ask about ${activeDataset.datasetName}…` : "Ask for any data…"}
//                 rows={1}
//                 className="flex-1 bg-transparent outline-none px-1 py-2 text-[15px] text-foreground placeholder:text-muted-foreground resize-none max-h-32 overflow-y-auto"
//               />
//               <Button onClick={() => sendMessage()} disabled={(!input.trim() && !attachedFile) || loading} size="icon" className="h-9 w-9 bg-gradient-to-br from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 rounded-xl text-white shadow-sm disabled:opacity-50 transition-all">
//                 {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
//               </Button>
//             </div>
//             <p className="text-[11px] text-muted-foreground text-center mt-2">Press Enter to send · Shift + Enter for new line</p>
...
//           <span className="font-bold text-xs">How they connect</span>
//         </div>
...
// // Helper: ensure dataset name has .csv extension
...
//   const schedStr = scheduleLabel();
//   return (
//     <div className="mt-2.5 max-w-[480px] rounded-xl border p-4" style={{ background: "linear-gradient(135deg, hsl(142 72% 42% / 0.07), hsl(197 100% 55% / 0.05))", border: "1.5px solid hsl(142 72% 42% / 0.35)" }}>
//       <div className="flex items-center gap-2.5 mb-3.5">
//         <div className="w-9 h-9 rounded-xl bg-green-600 flex items-center justify-center shrink-0"><CheckCircle className="w-4.5 h-4.5 text-white" style={{ width: 18, height: 18 }} /></div>
//         <div><div className="font-bold text-sm">Pipeline Created Successfully</div><div className="text-[11px] text-muted-foreground mt-0.5">{schedStr ? "Scheduled & ready" : "Ready to run on demand"}</div></div>
//       </div>
//       <div className="flex items-center gap-2 bg-card border border-border rounded-lg px-3 py-2 mb-2.5">
//         <GitBranch className="w-3.5 h-3.5 text-primary shrink-0" />
//         <div><div className="text-[10px] text-muted-foreground font-semibold mb-0.5">PIPELINE NAME</div><div className="text-sm font-bold">{pipeline.name}</div></div>
//       </div>
//       <div className="mb-2.5">
//         <div className="text-[11px] text-muted-foreground font-semibold mb-1.5">JOBS ({pipeline.jobs.length})</div>
//         {pipeline.jobs.map((jobId, idx) => (
//           <div key={jobId} className="flex items-center gap-2 bg-card border border-border rounded-lg px-2.5 py-1.5 mb-1">
//             <span className="text-[9px] font-bold text-primary bg-primary/12 rounded px-1.5 py-0.5">{idx + 1}</span>
//             <div><div className="text-xs font-semibold">{resolveJobName(jobId)}</div><div className="text-[10px] text-muted-foreground font-mono">{jobId.slice(0, 8)}…</div></div>
//           </div>
//         ))}
//       </div>
//       {schedStr && <div className="flex items-center gap-2 rounded-lg px-3 py-2" style={{ background: "hsl(197 100% 50% / 0.07)", border: "1px solid hsl(197 100% 50% / 0.25)" }}><Calendar className="w-3.5 h-3.5 shrink-0" style={{ color: "hsl(197 100% 38%)" }} /><div><div className="text-[10px] text-muted-foreground font-semibold mb-0.5">SCHEDULE</div><div className="text-xs font-semibold" style={{ color: "hsl(197 100% 35%)" }}>{schedStr}</div></div><div className="ml-auto"><span className="text-[10px] font-semibold text-white bg-green-600 rounded-full px-2 py-0.5">Active</span></div></div>}
//       <div className="mt-2.5 text-[10px] text-muted-foreground font-mono">ID: {pipeline.pipeline_id}</div>
//     </div>
//   );
// }
...
//         <button onClick={handleSaveJob} disabled={saving || saveStatus === "success"} className="flex items-center gap-1.5 text-xs font-semibold rounded-lg px-3.5 py-1.5 cursor-pointer border transition-all" style={{ color: saveStatus === "success" ? "#fff" : saveStatus === "error" ? "#fff" : "hsl(var(--foreground))", background: saveStatus === "success" ? "hsl(142 72% 38%)" : saveStatus === "error" ? "hsl(0 72% 51%)" : "hsl(var(--muted))", borderColor: "hsl(var(--border))" }}>
//           {saving ? <Loader2 className="w-3 h-3 animate-spin" /> : saveStatus === "success" ? <Check className="w-3 h-3" /> : <span>💾</span>}
//           {saving ? "Saving…" : saveStatus === "success" ? "Saved!" : "Save Dataset"}
//         </button>
//         {result.download_url && <button onClick={() => onDownload(result.download_url)} className="flex items-center gap-1.5 text-xs font-semibold text-white rounded-lg px-3.5 py-1.5 cursor-pointer border-none" style={{ background: "linear-gradient(135deg, hsl(267 84% 60%), hsl(220 90% 60%))" }}><span>⬇️</span> Download CSV</button>}
//       </div>
//     </div>
//   );
// }
...
//     onSelect({
//       datasetName: datasetNameWithExt,
//       jobName: ds.jobName,
//       job_id: ds.job_id,
//       dataset_path: datasetPath,
//       onelake_path: onelakePath,
...
//       {/* Side Drawer */}
//       <div className="fixed top-0 right-0 bottom-0 z-50 flex flex-col border-l border-border bg-background shadow-2xl transition-transform duration-300"
//         style={{ width: 380, transform: open ? "translateX(0)" : "translateX(100%)" }}>

//         {/* Header */}
//         <div className="shrink-0 px-5 pt-4 pb-3.5 border-b border-border bg-card">
//           <div className="flex items-center justify-between mb-1">
//             <div className="flex items-center gap-2.5">
//               <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: "linear-gradient(135deg, hsl(267 84% 60%), hsl(220 90% 60%))" }}>
//                 <Database className="w-4 h-4 text-white" />
//               </div>
//               <div>
//                 <div className="font-bold text-[15px] text-foreground">Your Datasets</div>
...
//             <button
//               onClick={onClose}
//               className="w-8 h-8 rounded-lg bg-muted border border-border flex items-center justify-center cursor-pointer text-muted-foreground hover:bg-accent hover:text-foreground transition-all"
//             >
//               <X className="w-3.5 h-3.5" />
//             </button>
//           </div>
...
//           {/* Active badge */}
//           {activeDataset && (
//             <div className="mt-2.5 flex items-center gap-1.5 rounded-lg px-2.5 py-1.5" style={{ background: "hsl(267 84% 60% / 0.08)", border: "1px solid hsl(267 84% 60% / 0.25)" }}>
//               <div className="w-1.5 h-1.5 rounded-full bg-green-500 shrink-0" />
//               <span className="text-[11px] text-muted-foreground">Active:</span>
//               <span className="text-[11px] font-bold text-primary flex-1 overflow-hidden text-ellipsis whitespace-nowrap">{activeDataset.datasetName}</span>
//               <span className="text-[10px] text-muted-foreground italic">(click to deselect)</span>
//             </div>
//           )}
//         </div>
...
//               onChange={(e) => setSearch(e.target.value)}
//               placeholder="Search datasets…"
//               className="w-full pl-8 pr-7 py-2 text-xs bg-muted/50 border border-border rounded-lg text-foreground outline-none focus:border-primary/50 transition-colors"
//             />
//             {search && (
//               <button onClick={() => setSearch("")} className="absolute right-2 top-1/2 -translate-y-1/2 bg-transparent border-none cursor-pointer p-0.5 flex items-center text-muted-foreground">
...
//               <span className="text-xs">Loading your datasets…</span>
//             </div>
//           ) : filtered.length === 0 ? (
//             <div className="text-center py-10 px-4">
//               <Database className="w-8 h-8 text-muted-foreground mx-auto mb-2.5" />
//               <div className="text-sm font-semibold text-foreground mb-1">{search ? "No matches found" : "No datasets yet"}</div>
//               <div className="text-[11px] text-muted-foreground">{search ? "Try a different term" : "Run a pipeline to generate datasets"}</div>
//             </div>
//           ) : (
//             filtered.map((ds, i) => {
//               const isActive = activeDataset?.datasetName === ds.datasetName && activeDataset?.job_id === ds.job_id;
...
//                   className="rounded-xl mb-2 overflow-hidden cursor-pointer transition-all select-none"
//                   style={{
//                     border: isActive ? "1.5px solid hsl(var(--primary))" : "1px solid hsl(var(--border))",
//                     background: isActive ? "hsl(267 84% 60% / 0.06)" : "hsl(var(--card))",
//                     boxShadow: isActive ? "0 0 0 3px hsl(267 84% 60% / 0.12)" : "none",
//                   }}
//                   onMouseEnter={(e) => { if (!isActive) (e.currentTarget as HTMLDivElement).style.borderColor = "hsl(var(--primary) / 0.4)"; }}
//                   onMouseLeave={(e) => { if (!isActive) (e.currentTarget as HTMLDivElement).style.borderColor = "hsl(var(--border))"; }}
//                 >
//                   {/* Card content */}
...
//                       {/* Icon */}
//                       <div className="w-9 h-9 rounded-xl shrink-0 flex items-center justify-center transition-all" style={{ background: isActive ? "linear-gradient(135deg, hsl(267 84% 60%), hsl(220 90% 60%))" : "hsl(var(--muted))" }}>
//                         <Table2 className="w-4 h-4" style={{ color: isActive ? "#fff" : "hsl(var(--muted-foreground))" }} />
//                       </div>

//                       {/* Info */}
//                       <div className="flex-1 min-w-0">
//                         <div className="flex items-center gap-1.5 mb-0.5">
//                           <span className="font-bold text-[13px] overflow-hidden text-ellipsis whitespace-nowrap max-w-[170px]" style={{ color: isActive ? "hsl(var(--primary))" : "hsl(var(--foreground))" }}>{ds.datasetName}</span>
//                           {isActive && <span className="text-[9px] font-bold text-white bg-primary rounded px-1.5 py-0.5 shrink-0">ACTIVE</span>}
//                         </div>
...
//                   {/* Preview button only (no Use this / Selected) */}
//                   <div className="flex border-t border-border/60 bg-muted/30">
//                     <button
//                       onClick={(e) => openPreview(e, ds)}
//                       className="flex-1 flex items-center justify-center gap-1.5 py-2 text-[11px] font-semibold text-muted-foreground bg-transparent border-none cursor-pointer hover:text-cyan-600 hover:bg-cyan-500/8 transition-all"
//                     >
//                       <Eye className="w-3 h-3" /> Preview
//                     </button>
//                     <div className="flex-1 flex items-center justify-center gap-1.5 py-2 text-[11px] font-semibold transition-all" style={{ color: isActive ? "hsl(142 72% 38%)" : "hsl(var(--muted-foreground))" }}>
//                       {isActive ? (
//                         <><CheckCircle className="w-3 h-3" /> Selected</>
//                       ) : (
//                         <><PlayCircle className="w-3 h-3" /> Click to select</>
//                       )}
//                     </div>
//                   </div>
//                 </div>
...
//         {/* Footer hint */}
//         {!loading && filtered.length > 0 && (
//           <div className="shrink-0 px-4 py-2.5 border-t border-border/50 bg-card">
...
//               <div className="flex items-center gap-2">
//                 {activeDataset?.datasetName !== previewTarget.datasetName && (
//                   <button onClick={() => { handleCardClick(previewTarget); setPreviewTarget(null); setPreviewData(null); setPreviewError(null); }} className="flex items-center gap-1.5 text-xs font-semibold text-white rounded-lg px-3.5 py-1.5 cursor-pointer border-none" style={{ background: "linear-gradient(135deg, hsl(267 84% 60%), hsl(220 90% 60%))" }}>
//                     <PlayCircle className="w-3 h-3" />Use this Dataset
//                   </button>
//                 )}
//                 <button onClick={() => { setPreviewTarget(null); setPreviewData(null); setPreviewError(null); }} className="bg-muted border border-border rounded-lg p-1.5 cursor-pointer flex items-center">
...
//               {previewLoading ? (
//                 <div className="flex items-center justify-center h-60 gap-3 text-muted-foreground text-sm"><Loader2 className="w-6 h-6 animate-spin" />Loading preview…</div>
//               ) : previewError ? (
//                 <div className="flex flex-col items-center justify-center h-60 gap-2.5"><AlertCircle className="w-7 h-7 text-red-500" /><div className="text-sm font-semibold text-red-500">Failed to load preview</div><div className="text-xs text-muted-foreground">{previewError}</div></div>
//               ) : previewData ? (
//                 <table className="w-full border-collapse text-xs">
//                   <thead className="sticky top-0 z-10">
//                     <tr className="bg-primary">
//                       {previewData.columns.map((col) => <th key={col} className="px-3.5 py-2.5 text-left font-semibold text-white whitespace-nowrap border-r border-white/10"><div className="text-xs">{col}</div><div className="text-[10px] opacity-75 font-normal mt-0.5">{previewData.column_types[col] || "?"}</div></th>)}
//                     </tr>
//                   </thead>
//                   <tbody>
//                     {previewData.preview_rows.length === 0 ? (
//                       <tr><td colSpan={previewData.columns.length} className="px-4 py-10 text-center text-muted-foreground text-sm">No preview rows available</td></tr>
//                     ) : (
//                       previewData.preview_rows.map((row, ri) => (
//                         <tr key={ri} className="border-b border-border/50 hover:bg-primary/4 transition-colors" style={{ background: ri % 2 === 0 ? "transparent" : "hsl(var(--muted) / 0.3)" }}>
//                           {previewData.columns.map((col) => <td key={col} className="px-3.5 py-2 text-foreground whitespace-nowrap border-r border-border/30">{row[col] != null ? String(row[col]) : <span className="text-muted-foreground">—</span>}</td>)}
//                         </tr>
//                       ))
//                     )}
//                   </tbody>
...
//   const handleDatasetSelect = (ds: ActiveDataset) => {
//     // Ensure dataset name and ALL paths use .csv extension
//     const datasetNameWithExt = withCsvExtension(ds.datasetName);
//     const datasetPath = ds.dataset_path.replace(/\/([^\/]+)$/, `/${datasetNameWithExt}`);
//     const onelakePath = ds.onelake_path
//       ? ds.onelake_path.replace(/\/([^\/]+)$/, `/${datasetNameWithExt}`)
//       : `Files/Datasets/${datasetPath}`;
...
//     setDatasetPanelOpen(false);
//     setMessages((prev) => [...prev, { id: Date.now().toString(), role: "assistant", content: `✅ **${datasetNameWithExt}** is now your active dataset. I'll use it for analysis, dashboards, and AutoML. What would you like to do with it?`, timestamp: new Date() }]);
//   };
...
//       const res = await fetch(`${BASE_URL}/run`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, job_id: jobId, prompt: content }) });
//       const data = await res.json();
//       // Store dataset name and ALL paths with .csv extension
//       const rawDatasetName = data.final_dataset?.dataset_name || "";
//       const datasetNameWithExt = withCsvExtension(rawDatasetName);
//       const rawDatasetPath = data.final_dataset?.dataset_path || "";
//       const rawOnelakePath = data.final_dataset?.onelake_path || "";
//       const datasetPath = rawDatasetPath
//         ? rawDatasetPath.replace(/\/([^\/]+)$/, `/${datasetNameWithExt}`)
//         : rawDatasetPath;
//       const onelakePath = rawOnelakePath
//         ? rawOnelakePath.replace(/\/([^\/]+)$/, `/${datasetNameWithExt}`)
//         : rawOnelakePath;
...
//       setMessages((prev) => [...prev, { id: (Date.now() + 1).toString(), role: "assistant", content: data.message || "Job completed.", result: { pipeline_name: data.pipeline_name, suggested_job_name: data.suggested_job_name || data.pipeline_name, job_name: data.job_name || "", job_id: data.job_id, data_model: data.data_model, relationships: data.relationships, schemas: data.schemas, final_dataset: { ...data.final_dataset, dataset_name: datasetNameWithExt }, download_url: data.download_url }, timestamp: new Date() }]);
//     } catch { setMessages((prev) => [...prev, { id: (Date.now() + 1).toString(), role: "assistant", content: "I couldn't reach the server. Please try again.", error: true, timestamp: new Date() }]); } finally { setLoading(false); }
//   };
...
//           activeDataset={activeDataset}
//           onSelect={handleDatasetSelect}
//           onDeselect={handleClearActiveDataset}
//         />
...
//               {activeDataset ? (
//                 <span className="max-w-[100px] overflow-hidden text-ellipsis whitespace-nowrap">{activeDataset.datasetName}</span>
//               ) : "Datasets"}
...
//               <button
//                 onClick={() => { setMessages([]); setPipelineStep("idle"); setPipelineSelectedJobs([]); }}
//                 className="flex items-center gap-1 text-xs font-medium text-muted-foreground bg-muted border border-border rounded-lg px-2.5 py-1.5 cursor-pointer hover:text-foreground transition-all"
//               >
...
//         <footer className="sticky bottom-0 z-20 bg-background/90 backdrop-blur-md border-t border-border">
//           <div className="max-w-3xl mx-auto px-4 py-3">
//             {/* Active dataset context line */}
//             {activeDataset && (
//               <div className="flex items-center gap-1.5 mb-1.5 text-[11px] text-muted-foreground">
//                 <div className="w-1.5 h-1.5 rounded-full bg-green-500 shrink-0" />
//                 <span>Working on:</span>
//                 <span className="font-bold text-primary">{activeDataset.datasetName}</span>
//                 <button onClick={() => setDatasetPanelOpen(true)} className="text-[10px] font-semibold text-muted-foreground bg-transparent border-none cursor-pointer underline underline-offset-2">Change</button>
//                 <button onClick={handleClearActiveDataset} className="flex items-center ml-0.5 bg-transparent border-none cursor-pointer text-muted-foreground p-0" title="Remove dataset">
//                   <X className="w-2.5 h-2.5" />
//                 </button>
//               </div>
//             )}
...
//                 style={{
//                   border: "1px solid",
//                   borderColor: activeDataset ? "hsl(var(--primary) / 0.4)" : "hsl(var(--border))",
//                   background: activeDataset ? "hsl(267 84% 60% / 0.1)" : "hsl(var(--muted))",
//                   color: activeDataset ? "hsl(var(--primary))" : "hsl(var(--muted-foreground))",
//                 }}
//               >
//                 <Database className="w-3 h-3" />
...
// ─────────────────────────────────────────────────────────────
// ChartRenderer — renders bar, donut/pie, scatter, table visuals
// ─────────────────────────────────────────────────────────────
function ChartRenderer({ visual }: { visual: DashboardVisual }) {
  ...
  if (visual.chart_type === "bar" && visual.data?.x) {
    const barData = visual.data.x.map((label: string, i: number) => ({
      label,
      ...(visual.data.series
        ? Object.fromEntries(
            Object.entries(visual.data.series).map(([k, vals]: [string, any]) => [k, vals[i]])
          )
    ...
    return (
      <ResponsiveContainer width="100%" height={h}>
        <BarChart data={barData} margin={{ top: 4, right: 8, bottom: 20, left: 8 }}>
          ...
          <Tooltip />
          {keys.map((k, i) => (
            <Bar key={k} dataKey={k} fill={DASHBOARD_COLORS[i % DASHBOARD_COLORS.length]} radius={[4, 4, 0, 0]} />
          ))}
        </BarChart>
      </ResponsiveContainer>
    );
  ...
  if ((visual.chart_type === "donut" || visual.chart_type === "pie") && visual.data?.labels) {
    const pieData = visual.data.labels.map((name: string, i: number) => ({
      name,
      value: visual.data.values?.[i] ?? 0,
    }));
    return (
      <ResponsiveContainer width="100%" height={h}>
        <PieChart>
          <Pie
            ...
            dataKey="value"
            nameKey="name"
            innerRadius={visual.chart_type === "donut" ? 60 : 0}
            outerRadius={90}
            paddingAngle={3}
          >
  ...
  if (visual.chart_type === "scatter" && visual.data?.x) {
    const scatterData = visual.data.x.map((x: number, i: number) => ({
      x,
      y: visual.data.y?.[i] ?? 0,
    }));
    return (
      <ResponsiveContainer width="100%" height={h}>
        <ScatterChart margin={{ top: 4, right: 8, bottom: 20, left: 8 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
          <XAxis dataKey="x" name={visual.x_axis_column} tick={{ fontSize: 11 }} label={{ value: visual.x_axis_column, position: "insideBottom", offset: -10, fontSize: 11 }} />
          <YAxis dataKey="y" name={visual.y_axis_columns?.[0]} tick={{ fontSize: 11 }} />
          <Tooltip cursor={{ strokeDasharray: "3 3" }} />
          <Scatter data={scatterData} fill={DASHBOARD_COLORS[0]} />
        </ScatterChart>
      </ResponsiveContainer>
    );
  }
      ...
      <div className="overflow-auto max-h-72">
        <table className="text-xs w-full">
          <thead className="bg-muted sticky top-0">
            <tr>
              {cols.map((c) => (
                <th key={c} className="border-b border-border px-2 py-1.5 text-left font-semibold text-muted-foreground whitespace-nowrap">{c}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {visual.data.rows.map((row: any, i: number) => (
              <tr key={i} className="border-b border-border last:border-0 hover:bg-muted/50" style={{ background: i % 2 === 0 ? "transparent" : "hsl(var(--muted) / 0.3)" }}>
                {cols.map((c) => (
                  <td key={c} className="px-2 py-1.5 text-foreground whitespace-nowrap">{String(row[c] ?? "—")}</td>
                ))}
              </tr>
            ))}
          </tbody>
  ...
  if (visual.chart_type === "line" && visual.data?.x) {
    const lineData = visual.data.x.map((label: string, i: number) => ({
      label,
      ...(visual.data.series
        ? Object.fromEntries(
            Object.entries(visual.data.series).map(([k, vals]: [string, any]) => [k, vals[i]])
    ...
    return (
      <ResponsiveContainer width="100%" height={h}>
        <LineChart data={lineData} margin={{ top: 4, right: 8, bottom: 20, left: 8 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
          <XAxis dataKey="label" tick={{ fontSize: 11 }} />
          <YAxis tick={{ fontSize: 11 }} />
          <Tooltip />
          {keys.map((k, i) => (
            <Line key={k} type="monotone" dataKey={k} stroke={DASHBOARD_COLORS[i % DASHBOARD_COLORS.length]} strokeWidth={2} dot={false} />
          ))}
        </LineChart>
      </ResponsiveContainer>
    );
  ...
  if (visual.chart_type === "area" && visual.data?.x) {
    const areaData = visual.data.x.map((label: string, i: number) => ({
      label,
      ...(visual.data.series
        ? Object.fromEntries(
            Object.entries(visual.data.series).map(([k, vals]: [string, any]) => [k, vals[i]])
    ...
    return (
      <ResponsiveContainer width="100%" height={h}>
        <AreaChart data={areaData} margin={{ top: 4, right: 8, bottom: 20, left: 8 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
          <XAxis dataKey="label" tick={{ fontSize: 11 }} />
          <YAxis tick={{ fontSize: 11 }} />
          <Tooltip />
          {keys.map((k, i) => (
            <Area key={k} type="monotone" dataKey={k} stroke={DASHBOARD_COLORS[i % DASHBOARD_COLORS.length]} fill={`${DASHBOARD_COLORS[i % DASHBOARD_COLORS.length]}33`} strokeWidth={2} />
          ))}
        </AreaChart>
      </ResponsiveContainer>
    );
...
// ─────────────────────────────────────────────────────────────
// PowerBIDashboardCard — FIXED: now renders all chart types
// ─────────────────────────────────────────────────────────────
function PowerBIDashboardCard({ dashboard }: { dashboard: DashboardResult }) {
  const navigate = useNavigate();
  const visuals = dashboard.visuals || [];
  ...
  return (
    <div className="mt-3 w-full max-w-2xl space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          onClick={() => navigate("/workflow/powerbi-flow")}
          className="flex items-center gap-1.5 text-xs font-semibold text-white rounded-lg px-3.5 py-1.5 cursor-pointer"
          style={{ background: "linear-gradient(135deg, #2563eb, #1d4ed8)", border: "1px solid rgba(255,255,255,0.15)" }}
        >
          <Share2 className="w-3 h-3" />Deploy to Power BI
        </button>
      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {kpiVisuals.map((kpi: any, i: number) => {
              ...
              return (
                <div key={i} className="bg-card border border-border rounded-xl p-4">
                  <p className="text-xs text-muted-foreground font-medium mb-1">{kpi.chart_name}</p>
                  <p className="text-xl font-bold text-primary mb-1">{formatted}</p>
                  <p className="text-[10px] text-muted-foreground">{kpi.description || "Result from query"}</p>
                </div>
              );
            })}
          </div>
      ...
      {/* Chart visuals — previously missing, now rendered */}
      {chartVisuals.length > 0 && (
        <div>
          ...
          <div className="space-y-4">
            {chartVisuals.map((v: any, i: number) => (
              <div key={i} className="bg-card border border-border rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-xs font-semibold text-foreground">{v.chart_name}</p>
                  <span className="text-[10px] text-muted-foreground bg-muted rounded px-2 py-0.5 capitalize">{v.chart_type}</span>
                </div>
                {v.description && (
      ...
      {/* Empty state */}
      {!kpiVisuals.length && !chartVisuals.length && (
        <div className="text-center py-10">
          <AlertCircle className="w-12 h-12 text-muted-foreground mx-auto mb-3" />
          <p className="text-sm text-foreground">No results generated</p>
        </div>
...
// ─────────────────────────────────────────────────────────────
// DatasetPanel — inline panel inside chat body, right side
// FIXED: absolute positioned inside chat container, no backdrop,
//        only Preview button on cards, click card to select/deselect
// ─────────────────────────────────────────────────────────────
function DatasetPanel({
  ...
  return (
    <>
      {/* Panel — absolutely positioned inside chat container */}
      <div
        className="flex flex-col bg-background border-l border-b border-border"
        style={{
          position: "absolute",
          top: 57,
          right: 0,
      ...
      >
        ...
        <div className="shrink-0 px-4 pt-3.5 pb-3 border-b border-border bg-card">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="font-bold text-[14px] text-foreground">Your Datasets</div>
            ...
            <button
              onClick={onClose}
              className="w-7 h-7 rounded-lg bg-muted border border-border flex items-center justify-center cursor-pointer text-muted-foreground hover:bg-accent transition-all"
            >
              <X className="w-3 h-3" />
            </button>
          </div>
          ...
          {activeDataset && (
            <div
              ...
              style={{ background: "hsl(267 84% 60% / 0.08)", border: "1px solid hsl(267 84% 60% / 0.25)" }}
            >
              <div className="w-1.5 h-1.5 rounded-full bg-green-500 shrink-0" />
              <span className="text-[11px] text-muted-foreground">Active:</span>
              <span className="text-[11px] font-bold text-primary flex-1 overflow-hidden text-ellipsis whitespace-nowrap">
                {activeDataset.datasetName}
              </span>
              <span className="text-[10px] text-muted-foreground italic shrink-0">(click again to deselect)</span>
            </div>
          )}
        </div>
        ...
        <div className="shrink-0 px-3 py-2.5 border-b border-border/50">
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search datasets…"
              className="w-full pl-8 pr-7 py-1.5 text-xs bg-muted/50 border border-border rounded-lg text-foreground outline-none focus:border-primary/50 transition-colors"
            />
            {search && (
              <button
                onClick={() => setSearch("")}
                className="absolute right-2 top-1/2 -translate-y-1/2 bg-transparent border-none cursor-pointer p-0.5 flex items-center text-muted-foreground"
              >
                <X className="w-2.5 h-2.5" />
              </button>
            )}
        ...
        <div className="flex-1 overflow-y-auto p-3">
          {loading ? (
            ...
            <div className="text-center py-8 px-4">
              ...
              <div className="text-[11px] text-muted-foreground">
                {search ? "Try a different term" : "Run a pipeline to generate datasets"}
              </div>
            </div>
          ) : (
            filtered.map((ds, i) => {
              const isActive =
                activeDataset?.datasetName === ds.datasetName &&
                activeDataset?.job_id === ds.job_id;
              return (
                <div
                  key={i}
                  onClick={() => handleCardClick(ds)}
                  ...
                  style={{
                    ...
                    background: isActive
                      ? "hsl(267 84% 60% / 0.06)"
                      : "hsl(var(--card))",
                    boxShadow: isActive ? "0 0 0 3px hsl(267 84% 60% / 0.10)" : "none",
                  }}
                  onMouseEnter={(e) => {
                    if (!isActive)
                ...
                >
                  ...
                  <div className="px-3 py-2.5">
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          <span
                            className="font-bold text-[12px] overflow-hidden text-ellipsis whitespace-nowrap max-w-[160px]"
                            style={{
                              color: isActive
                                ? "hsl(var(--primary))"
                                : "hsl(var(--foreground))",
                  ...
                  {/* Preview button only — no "Click to select" text */}
                  <div className="border-t border-border/60 bg-muted/30">
                    <button
                      onClick={(e) => openPreview(e, ds)}
                      className="w-full flex items-center justify-center gap-1.5 py-1.5 text-[11px] font-semibold text-muted-foreground bg-transparent border-none cursor-pointer hover:text-cyan-600 hover:bg-cyan-500/8 transition-all"
                    >
                      <Eye className="w-3 h-3" /> Preview
                    </button>
                  </div>
        ...
        {/* Footer hint */}
        {!loading && filtered.length > 0 && (
          <div className="shrink-0 px-4 py-2 border-t border-border/50 bg-card">
      ...
      {previewTarget &&
        createPortal(
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-6">
            ...
            />
            <div
              className="relative bg-background border border-border rounded-2xl flex flex-col shadow-2xl overflow-hidden"
              style={{ width: "90vw", maxWidth: 920, maxHeight: "82vh" }}
            >
              <div className="shrink-0 px-5 py-4 border-b border-border flex items-center justify-between bg-card">
                <div>
                  <div className="font-bold text-base mb-0.5">Dataset Preview</div>
                  <div className="text-xs text-muted-foreground flex items-center gap-1.5">
                    <span className="text-primary font-semibold bg-primary/10 border border-primary/20 rounded px-2 py-0.5 text-[11px]">
                      {previewTarget.datasetName}
                    </span>
                    {previewData && (
                      <span>
                ...
                <div className="flex items-center gap-2">
                  {activeDataset?.datasetName !== previewTarget.datasetName && (
                    <button
                      onClick={() => {
                        handleCardClick(previewTarget);
                        setPreviewTarget(null);
                        setPreviewData(null);
                        setPreviewError(null);
                      }}
                      className="flex items-center gap-1.5 text-xs font-semibold text-white rounded-lg px-3.5 py-1.5 cursor-pointer border-none"
                      style={{
                        background:
                          "linear-gradient(135deg, hsl(267 84% 60%), hsl(220 90% 60%))",
                      }}
                  ...
                  <button
                    ...
                    className="bg-muted border border-border rounded-lg p-1.5 cursor-pointer flex items-center"
                  >
                    <X className="w-3.5 h-3.5 text-muted-foreground" />
                  </button>
                </div>
              ...
              <div className="flex-1 overflow-auto">
                {previewLoading ? (
                  <div className="flex items-center justify-center h-60 gap-3 text-muted-foreground text-sm">
                    <Loader2 className="w-6 h-6 animate-spin" />Loading preview…
                  </div>
                ) : previewError ? (
                  <div className="flex flex-col items-center justify-center h-60 gap-2.5">
                    <AlertCircle className="w-7 h-7 text-red-500" />
                    <div className="text-sm font-semibold text-red-500">Failed to load preview</div>
                    ...
                    <thead className="sticky top-0 z-10">
                      <tr className="bg-primary">
                        {previewData.columns.map((col) => (
                          <th
                            key={col}
                            className="px-3.5 py-2.5 text-left font-semibold text-white whitespace-nowrap border-r border-white/10"
                          >
                    ...
                    <tbody>
                      {previewData.preview_rows.length === 0 ? (
                        ...
                        previewData.preview_rows.map((row, ri) => (
                          <tr
                            key={ri}
                            className="border-b border-border/50 hover:bg-primary/4 transition-colors"
                            style={{
                              background:
                                ri % 2 === 0
                                  ? "transparent"
                          ...
                          >
                            {previewData.columns.map((col) => (
                              <td
                                key={col}
                                className="px-3.5 py-2 text-foreground whitespace-nowrap border-r border-border/30"
                              >
                                {row[col] != null ? (
                                  String(row[col])
                                ) : (
...
export default function VeritonChatBot() {
  ...
  const handleDatasetSelect = (ds: ActiveDataset) => {
    ...
    setMessages((prev) => [
      ...prev,
      {
        id: Date.now().toString(),
        role: "assistant",
        content: `✅ **${datasetNameWithExt}** is now your active dataset. I'll use it for analysis, dashboards, and AutoML. What would you like to do with it?`,
        timestamp: new Date(),
      },
    ]);
  };
  ...
  const isGreeting = (t: string) =>
    ["hi", "hello", "hey", "good morning", "good afternoon", "good evening"].some(
      (g) =>
        t.toLowerCase().trim() === g ||
        t.toLowerCase().startsWith(g + " ") ||
        t.toLowerCase().startsWith(g + "!") ||
        t.toLowerCase().startsWith(g + ",")
    );
  const isPowerBiIntent = (t: string) => {
    const l = t.toLowerCase();
    return l.includes("powerbi") || l.includes("power bi") || l.includes("dashboard") || l.includes("report") || l.includes("visualize");
  };
  const isAutoMLIntent = (t: string) => {
    const l = t.toLowerCase();
    return l.includes("build model") || l.includes("train model") || l.includes("ml model") || l.includes("machine learning") || l.includes("automl") || l.includes("regression") || l.includes("classification") || l.includes("predict");
  };
  const isCreatePipelineIntent = (t: string) => t.toLowerCase().includes("create pipeline");
  const isDatasetIntent = (t: string) => {
    const l = t.toLowerCase();
    return l.includes("select dataset") || l.includes("choose dataset") || l.includes("switch dataset") || l.includes("my datasets") || l.includes("show datasets") || l.includes("use dataset");
  };
  ...
  const handleCreatePipeline = async (content: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now().toString(), role: "assistant", content: data.response || "Select jobs for pipeline:", pipelineJobs: jobs, timestamp: new Date() },
      ]);
      setPipelineStep("awaiting_job_selection");
    } catch {
      setMessages((prev) => [
        ...prev,
        { id: Date.now().toString(), role: "assistant", content: "Failed to start pipeline creation.", error: true, timestamp: new Date() },
      ]);
      setPipelineStep("idle");
    } finally {
      setLoading(false);
  ...
  const handleJobSelectionConfirm = async (selectedIds: string[]) => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: Date.now().toString(), role: "user", content: `Selected ${selectedIds.length} job(s) for pipeline.`, timestamp: new Date() },
    ]);
    setLoading(true);
    try {
      const res = await fetch(`${BASE_URL}/chat`, {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now().toString(), role: "assistant", content: data.response || "Please enter a pipeline name.", timestamp: new Date() },
      ]);
      setPipelineStep("awaiting_pipeline_name");
    } catch {
      setMessages((prev) => [
        ...prev,
        { id: Date.now().toString(), role: "assistant", content: "Failed to confirm job selection.", error: true, timestamp: new Date() },
      ]);
      setPipelineStep("idle");
    } finally {
      setLoading(false);
  ...
  const handlePipelineNameSubmit = async (content: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now().toString(), role: "assistant", content: data.response || "Pipeline created!", pipelineCreated, timestamp: new Date() },
      ]);
      setPipelineStep("idle");
      setPipelineSelectedJobs([]);
    } catch {
      setMessages((prev) => [
        ...prev,
        { id: Date.now().toString(), role: "assistant", content: "Failed to create pipeline.", error: true, timestamp: new Date() },
      ]);
      setPipelineStep("idle");
    } finally {
      setLoading(false);
  ...
  const sendMessage = async (text?: string) => {
    ...
    if (isGreeting(content)) {
      ...
      setTimeout(() => setMessages((prev) => [
        ...prev,
        { id: (Date.now() + 1).toString(), role: "assistant", content: "Hello! 👋 How can I help you with your data today?", timestamp: new Date() },
      ]), 600);
      return;
    }
    if (isDatasetIntent(content)) { setLoading(false); setDatasetPanelOpen(true); return; }
    ...
    if (isAutoMLIntent(content)) {
      try {
        ...
        const res = await fetch(`${BASE_URL}/automl/run`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ user_id: userId, job_id: jobId, session_id: aivolveUser?.session_id || "", user_email: aivolveUser?.email || "", query: content }),
        });
        const data = await res.json();
        setMessages((prev) => [
          ...prev,
          { id: (Date.now() + 1).toString(), role: "assistant", content: data.message || "AutoML completed!", automlResult: data, timestamp: new Date() },
        ]);
      } catch {
        setMessages((prev) => [
          ...prev,
          { id: (Date.now() + 1).toString(), role: "assistant", content: "Failed to run AutoML.", error: true, timestamp: new Date() },
        ]);
      } finally { setLoading(false); }
      return;
    }
    ...
    if (isPowerBiIntent(content)) {
      try {
        ...
        if (!csvBlob) {
          setMessages((prev) => [
            ...prev,
            { id: (Date.now() + 1).toString(), role: "assistant", content: "No dataset selected. Please select a dataset first using the **Datasets** button.", error: true, timestamp: new Date() },
          ]);
          setLoading(false);
          return;
        }
        ...
        sessionStorage.setItem(
          ...
          JSON.stringify({ ...data, file_name: (localStorage.getItem("current_dataset_path") || "").split("/").pop() || "" })
        );
        setMessages((prev) => [
          ...prev,
          { id: (Date.now() + 1).toString(), role: "assistant", content: "Power BI dashboard generated!", dashboardResult: data, timestamp: new Date() },
        ]);
      } catch {
        setMessages((prev) => [
          ...prev,
          { id: (Date.now() + 1).toString(), role: "assistant", content: "Failed to generate the dashboard.", error: true, timestamp: new Date() },
        ]);
      } finally { setLoading(false); }
      return;
    }
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          result: {
            ...
            relationships: data.relationships,
            schemas: data.schemas,
            final_dataset: { ...data.final_dataset, dataset_name: datasetNameWithExt },
            download_url: data.download_url,
          },
          timestamp: new Date(),
        },
      ...
      setMessages((prev) => [
        ...prev,
        { id: (Date.now() + 1).toString(), role: "assistant", content: "I couldn't reach the server. Please try again.", error: true, timestamp: new Date() },
      ]);
    } finally { setLoading(false); }
  };
  ...
  return (
    <WorkflowLayout>
      {/*
        IMPORTANT: position: relative here so the dataset panel
        (position: absolute) is scoped inside this container.
      */}
      <div className="flex flex-col h-screen bg-background relative overflow-hidden">

        {/* ── Header ── */}
        <header
          className="sticky top-0 z-20 flex items-center justify-between px-6 py-3.5 bg-background/90 backdrop-blur-md border-b border-border"
          style={{ height: 57 }}
        >
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-9 h-9 rounded-xl bg-gradient-to-br from-purple-500 to-indigo-600 shadow-md shadow-purple-500/20">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <ChevronRight
                ...
                style={{ transform: datasetPanelOpen ? "rotate(180deg)" : "none" }}
              />
            </button>
        ...
        {/* ── Dataset Panel — absolute inside this container ── */}
        {datasetPanelOpen && (
          <DatasetPanel
            userId={userId}
            activeDataset={activeDataset}
              ...
              <div className="space-y-5">
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                    ...
                    <div className={`flex flex-col max-w-[78%] ${msg.role === "user" ? "items-end" : "items-start"}`}>
                      <div
                        className={`px-4 py-2.5 rounded-2xl text-[15px] leading-relaxed whitespace-pre-wrap break-words shadow-sm ${
                          msg.role === "user"
                ...
                {loading && (
                  <div className="flex gap-3">
                    <div className="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 shadow-sm shrink-0">
                      <Bot className="w-4 h-4 text-white" />
                    </div>
                    <div className="px-4 py-3 bg-card border border-border rounded-2xl rounded-tl-sm shadow-sm">
                      <div className="flex items-center gap-1">
        ...
        {/* ── Footer / Input ── */}
        <footer
          className="sticky bottom-0 z-20 bg-background/90 backdrop-blur-md border-t border-border"
          style={{ height: 65 }}
        >
          <div className="max-w-3xl mx-auto px-4 py-3">
            {/* Active dataset context line */}
            {activeDataset && (
              <div className="flex items-center gap-1.5 mb-1.5 text-[11px] text-muted-foreground">
                ...
                <span className="font-bold text-primary">{activeDataset.datasetName}</span>
                <button
                  onClick={() => setDatasetPanelOpen(true)}
                  className="text-[10px] font-semibold text-muted-foreground bg-transparent border-none cursor-pointer underline underline-offset-2"
                >
                  Change
                </button>
                <button
                  onClick={handleClearActiveDataset}
                  className="flex items-center ml-0.5 bg-transparent border-none cursor-pointer text-muted-foreground p-0"
            ...
            {attachedFile && (
              <div className="mb-2 flex items-center gap-2 px-3 py-2 bg-primary/10 border border-primary/20 rounded-lg">
                <FileText className="w-4 h-4 text-primary shrink-0" />
                <span className="text-sm text-foreground truncate flex-1">{attachedFile.name}</span>
                <button onClick={removeAttachment} className="p-1 rounded-md hover:bg-primary/10 text-muted-foreground">
                  <X className="w-3.5 h-3.5" />
                </button>
              </div>
            )}
